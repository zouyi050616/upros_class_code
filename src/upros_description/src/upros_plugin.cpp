#include <upros_description/upros_plugin.h>

namespace gazebo
{

  UPROSPlugin::UPROSPlugin() {}

  UPROSPlugin::~UPROSPlugin() {}

  void UPROSPlugin::Load(physics::ModelPtr parent, sdf::ElementPtr sdf)
  {
    parent_ = parent;
    ...
    {
      ROS_INFO("UPROSPlugin missing <robotNamespace>, defaults to \"%s\"", robot_namespace_.c_str());
    }
    else
    {
    ...
    {
      ROS_WARN("UPROSPlugin (ns = %s) missing <commandTopic>, defaults to \"%s\"", robot_namespace_.c_str(), command_topic_.c_str());
    }
    else
    {
    ...
    {
      ROS_WARN("UPROSPlugin (ns = %s) missing <odometryTopic>, defaults to \"%s\"", robot_namespace_.c_str(), odometry_topic_.c_str());
    }
    else
    {
    ...
    odometry_frame_ = "odom_combined";
    if (!sdf->HasElement("odometryFrame"))
    {
      ROS_WARN("UPROSPlugin (ns = %s) missing <odometryFrame>, defaults to \"%s\"", robot_namespace_.c_str(), odometry_frame_.c_str());
    }
    else
    {
    ...
    torque_yaw_velocity_p_gain_ = 50.0;
    ...
    ROS_INFO_STREAM("UPROSPlugin using gains: yaw: " << torque_yaw_velocity_p_gain_ << " x: " << force_x_velocity_p_gain_ << " y: " << force_y_velocity_p_gain_ << "\n");

    robot_base_frame_ = "base_footprint";
    if (!sdf->HasElement("robotBaseFrame"))
    {
      ROS_WARN("UPROSPlugin (ns = %s) missing <robotBaseFrame>, defaults to \"%s\"", robot_namespace_.c_str(), robot_base_frame_.c_str());
    }
    else
    {
    ...
    ROS_INFO_STREAM("robotBaseFrame for UPROSPlugin plugin: " << robot_base_frame_ << "\n");
    ...
    {
      ROS_WARN("UPROSPlugin (ns = %s) missing <odometryRate>, defaults to %f", robot_namespace_.c_str(), odometry_rate_);
    }
    else
    {
    ...
    {
      ROS_WARN("UPROSPlugin Plugin (ns = %s) missing <publishOdometryTf>, defaults to %s", this->robot_namespace_.c_str(), this->publish_odometry_tf_ ? "true" : "false");
    }
    else
    {
    ...
    {
      ROS_FATAL_STREAM("UPROSPlugin (ns = " << robot_namespace_
    ...
    ROS_INFO("UPROSPlugin (%s) has started!",
    ...
    ros::SubscribeOptions so =
        ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
                                                            boost::bind(&UPROSPlugin::cmdVelCallback, this, _1),
                                                            ros::VoidPtr(), &queue_);
    ...
    callback_queue_thread_ = boost::thread(boost::bind(&UPROSPlugin::QueueThread, this));

    update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&UPROSPlugin::UpdateChild, this));
  }

  void UPROSPlugin::UpdateChild()
  {
  ...
  void UPROSPlugin::cmdVelCallback(const geometry_msgs::Twist::ConstPtr &cmd_msg)
  {
    boost::mutex::scoped_lock scoped_lock(lock);
    x_ = cmd_msg->linear.x;
    y_ = cmd_msg->linear.y;
  ...
  void UPROSPlugin::publishOdometry(double step_time)
  {
  ...
  tf::Transform UPROSPlugin::getTransformForMotion(double linear_vel_x, double linear_vel_y, double angular_vel, double timeSeconds) const
  {
    tf::Transform tmp;
    tmp.setIdentity();
  ...
  GZ_REGISTER_MODEL_PLUGIN(UPROSPlugin)
}