...
int main(int argc, char **argv)
{
  ...
  { // 循环直到ROS节点停止
    ...
    try
    {
      listener.lookupTransform("base_link", "laser_link", ros::Time(0), transform); // 获取base_link在map坐标系下的变换
    }
    catch (tf::TransformException ex)
    {                             // 如果获取变换失败，打印错误信ROS_ERROR("%s",ex.what());
      ros::Duration(1.0).sleep(); // 等待1秒再继续continue;
    }
    ...
    ROS_INFO("laser_link in base_link coordinate system: x = %f, y = %f, z = %f, roll = %f, pitch = %f, yaw = %f",
