...
class RobotCommunication:
    ...
    def parse_data(self, data_size, data):
        if data[0] == data_frame.HEAD1 and data[1] == data_frame.HEAD2:
            ...
            # 舵机数据返回
            if data[2] == data_frame.ID_GET_SERVO_INFO and data[5] == 0:
                servo_id = data[4]
                servo_pos = int.from_bytes(data[6:10], byteorder='little', signed=True)
                if self.servo_callback is not None:
                    self.servo_callback(servo_id, servo_pos)
    ...
    def get_servo_info(self):
        for i in range(20):
            data = [0] * 1
            data[0] = i + 1
            data_pack = self.generateCmd(data_frame.ID_GET_SERVO_INFO, data)
            self.msg_list.append(data_pack)
    ...
    def single_servo_control(self, servo_id, servo_pos, servo_speed):
        ...
        data = id + angle + speed
        data_pack = self.generateCmd(data_frame.ID_SINGLE_SERVO, data)
        self.msg_list.append(data_pack)
